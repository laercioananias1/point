 ...
 * padrão do convite de assinatura do aluno). Se ainda não tem conta, cria
 * a senha na hora; se já tem, só faz login e confirma — nos dois casos o
 * vínculo ativa sozinho assim que aceita. */
export default function ConviteVinculoAceite() {
  const { token } = useParams<{ token: string }>();
  const navigate = useNavigate();
  const { loginComToken } = useAuth();
  const [convite, setConvite] = useState<ConviteVinculo | null>(null);
      ...
      <div>
        ...
        {!carregando && convite && (
          <>
            ...
            {aceito ? (
              <p className="auth-card form-success">Vínculo ativado! Redirecionando...</p>
            ...
            ) : convite.professor_ja_cadastrado ? (
              <AceitarComLogin
                token={token!}
                onAceito={() => {
                  setAceito(true);
...
function ResumoConvite({ convite }: { convite: ConviteVinculo }) {
  return (
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await api.post<{ access_token: string; user: User }>(
        `/convites-vinculo/${token}/aceitar-novo`,
        { senha },
      );
...
function AceitarComLogin({ token, onAceito }: { token: string; onAceito: () => void }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      await api.post(`/convites-vinculo/${token}/aceitar`);
      onAceito();
    } catch (e) {
