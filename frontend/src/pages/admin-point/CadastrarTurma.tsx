...
/** Tela do admin pra criar turma em nome de qualquer professor vinculado
 * ao seu Point (pedido do usuário, 2026-09-09: "o adm tb pode criar
 * turmas porém precisa selecionar o professor") — mesmo formulário de
 * professor/CadastrarTurma.tsx, só que aqui quem escolhe é o PROFESSOR
 * (o Point já é fixo, é o do admin), não o Point. */
export default function AdminPointCadastrarTurma() {
  ...
  useEffect(() => {
    api
      .get<Vinculo[]>("/vinculos")
      .then((res) => setVinculos(res.filter((v) => v.status === "ativo")))
      .catch(() => setErroCarregar("Não foi possível carregar os professores. Tente novamente."))
    ...
    <Layout>
      <div className="screen-header">
        <button
          type="button"
          className="close-btn"
          onClick={() => navigate("/admin-point/turmas")}
          aria-label="Voltar"
        >
        ...
        <>
          {vinculos.length === 0 ? (
            <p className="form-error">
              Nenhum professor com vínculo ativo nesse Point ainda — convide um professor antes.
            </p>
          ) : (
            <CriarTurmaForm
              vinculos={vinculos}
              onCriada={(mensagem) => navigate("/admin-point/turmas", { state: { criada: mensagem } })}
            />
          )}
        </>
      )}
...
function CriarTurmaForm({
  ...
  if (tiposTurma.length === 0) {
    return (
      <p className="form-error">
        Nenhum tipo de turma cadastrado nesse Point ainda — cadastre pelo menos um tipo (ex.:
        "Padrão") antes de criar uma turma.
      </p>
    );
  }
  ...
  if (modalidades.length === 0) {
    return (
      <p className="form-error">
        Nenhuma modalidade cadastrada nesse Point ainda — cadastre modalidades e quadras antes.
      </p>
    );
  }
  ...
  if (categorias.length === 0) {
    return (
      <p className="form-error">
        Nenhuma categoria cadastrada nesse Point ainda — cadastre as categorias (níveis) antes de
        criar uma turma.
      </p>
    );
  }
  ...
  return (
    <form className="form-card" onSubmit={handleSubmit} style={{ maxWidth: "none" }}>
      ...
      {quadras.length === 0 ? (
        <p className="form-error">
          Nenhuma quadra cadastrada pra essa modalidade — associe uma antes.
        </p>
      ) : (
